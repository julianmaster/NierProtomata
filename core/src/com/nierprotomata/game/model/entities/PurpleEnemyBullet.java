...
public class PurpleEnemyBullet extends Entity {

    private final float degree;

    public PurpleEnemyBullet(GameScreen screen, Polygon shape, float degree) {
        super(screen, shape);
        this.degree = degree;
    ...
    public void render(SpriteBatch batch) {
        ...
        TextureRegion player = TextureManager.getTexture(Assets.PURPLE_BULLET.ordinal());
        batch.draw(player, shape.getX(), shape.getY());
    }

    @Override
    public void triggerCollision(Entity otherCollider) {
        if(otherCollider instanceof Wall || otherCollider instanceof Player) {
